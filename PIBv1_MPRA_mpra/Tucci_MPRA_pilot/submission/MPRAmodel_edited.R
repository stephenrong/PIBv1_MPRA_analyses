...
plot_logFC <- function(full_output, sample, negCtrlName="negCtrl", posCtrlName="expCtrl", emvarCtrlName="emvarCtrl", color_table) {  ###
  ...
  tmp_plotB<-ggplot(exp_values,aes(x=ctrl_mean,y=log2FoldChange,color=ctrl_exp)) +
    ...
    geom_point(data = subset(exp_values, ctrl_exp == emvarCtrlName), aes(x=ctrl_mean,y=log2FoldChange),alpha = .8,size=2) +  ###
    scale_x_log10() +
    #coord_cartesian(xlim = c(10, 1000),ylim = c(-1.5,7.5)) +
    xlab("Normalized Tag Count - Plasmids") + ylab(paste0(sample," Expression Fold Change log2(RNA/Plasmid)")) +
...
MPRAmodel <- function(countsData, attributesData, conditionData, filePrefix, negCtrlName="negCtrl", posCtrlName="expCtrl", emvarCtrlName="emvarCtrl", projectName="MPRA_PROJ", exclList=c(), plotSave=T, altRef=T, method = 'ss', tTest=T, DEase=T, cSkew=T, correction="BH", cutoff=0.01, upDisp=T, prior=F, raw=T, paired=F, color_table, ...){  ###
  ...
  rep1_loc <- grep("_rep1", colnames(counts_out))  ###
  ...
  for (celltype in levels(cond_data$condition)) {
    ...
    plot_list<-plot_logFC(output_tmp, celltype, negCtrlName, posCtrlName, emvarCtrlName, color_table)  ###
    if(plotSave==F){
      plot_list[[1]]
      plot_list[[2]]
    }
