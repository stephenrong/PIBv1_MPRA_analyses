...
corehaps_variants <- GRanges(as_tibble(fread("../../results/3-merge_all_variant_annotations/corehaps_variants_all_annotations_summ.txt.gz")))
names(mcols(corehaps_variants))[1:5] <- paste0("MPRA_", names(mcols(corehaps_variants))[1:5])

# get emVar 
mpra_emVars_variants <- corehaps_variants %>% 
