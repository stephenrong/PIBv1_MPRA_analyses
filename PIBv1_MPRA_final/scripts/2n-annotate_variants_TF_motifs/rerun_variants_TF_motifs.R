...
# load variants
introgressed_variants <- as_tibble(readRDS("../../results/1a-preprocess_PIBv1_MPRA_final/introgressed_variants.rds"))[,1:10]
...
for (k in i:j) {
	...
	introgressed_variants_TF_motifs_hocomoco <- motifbreakR(
		snpList = introgressed_variants_TF_motifs[k,],  # snps.mb.temp,
		filterp = TRUE,
		pwmList = hocomoco, 
		threshold = 1e-4, 
		method = "ic",
	...
	saveRDS(introgressed_variants_TF_motifs_hocomoco, paste0("../../results/2n-annotate_variants_TF_motifs/motifbreakR_job_outputs/introgressed_variants_TF_motifs_hocomoco-", k, ".rds"))
}
...
for (k in i:j) {
	print(k)
	introgressed_variants_TF_motifs_jaspar <- motifbreakR(
		snpList = introgressed_variants_TF_motifs[k,],  # snps.mb.temp,
		filterp = TRUE,
		pwmList = jaspar, 
		threshold = 1e-4, 
	...
	saveRDS(introgressed_variants_TF_motifs_jaspar, paste0("../../results/2n-annotate_variants_TF_motifs/motifbreakR_job_outputs/introgressed_variants_TF_motifs_jaspar-", k, ".rds"))
}
