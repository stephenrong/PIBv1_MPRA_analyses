...
# ensembl vep order
introgressed_variants_Ensembl_VEP_order <- c("transcript_ablation", "splice_acceptor_variant", "splice_donor_variant", "stop_gained", "frameshift_variant", "stop_lost", "start_lost", "transcript_amplification", "inframe_insertion", "inframe_deletion", "missense_variant", "protein_altering_variant", "splice_region_variant", "splice_donor_5th_base_variant", "splice_donor_region_variant", "splice_polypyrimidine_tract_variant", "incomplete_terminal_codon_variant", "start_retained_variant", "stop_retained_variant", "synonymous_variant", "coding_sequence_variant", "mature_miRNA_variant", "5_prime_UTR_variant", "3_prime_UTR_variant", "non_coding_transcript_exon_variant", "intron_variant", "NMD_transcript_variant", "non_coding_transcript_variant", "upstream_gene_variant", "downstream_gene_variant", "TFBS_ablation", "TFBS_amplification", "TF_binding_site_variant", "regulatory_region_ablation", "regulatory_region_amplification", "feature_elongation", "regulatory_region_variant", "feature_truncation", "intergenic_variant")

# ensembl vep names
introgressed_variants_Ensembl_VEP_names <- c("Uploaded_variation", "Location", "Allele", "Gene", "Feature", "Feature_type", "Consequence", "cDNA_position", "CDS_position", "Protein_position", "Amino_acids", "Codons", "Existing_variation", "Extra", "IMPACT", "SYMBOL", "SYMBOL_SOURCE", "STRAND", "BIOTYPE")

# load VCF 
introgressed_variants_Ensembl_VEP <- as_tibble(read.vcfR("../../results/2a-annotate_variants_Ensembl_VEP/introgressed_variants-Ensembl_VEP_nopick.vcf.gz")@fix) 

# add GRange and Variant cols
introgressed_variants_Ensembl_VEP <- introgressed_variants_Ensembl_VEP %>% 
...
introgressed_variants_Ensembl_VEP <- introgressed_variants_Ensembl_VEP %>% 
	...
	separate(col=INFO, into=paste("Ensembl_VEP_orig", introgressed_variants_Ensembl_VEP_names, sep="_"), remove=TRUE, sep="\\|") 
...
transId_map <- deframe(transId(sort(unique(introgressed_variants_Ensembl_VEP$Ensembl_VEP_orig_Gene)), transTo=c("symbol"), keepNA=TRUE, unique=TRUE, hgVersion="v38"))
introgressed_variants_Ensembl_VEP <- introgressed_variants_Ensembl_VEP %>% 
	rowwise() %>% mutate(Ensembl_VEP_temp_Symbol = ifelse(Ensembl_VEP_orig_Gene != "", transId_map[[Ensembl_VEP_orig_Gene]], NA)) %>% ungroup()

# save names
introgressed_variants_Ensembl_VEP_names <- names(introgressed_variants_Ensembl_VEP)[11:length(introgressed_variants_Ensembl_VEP)]
...
introgressed_variants_Ensembl_VEP <- introgressed_variants_Ensembl_VEP %>% 
	...
	arrange(match(Ensembl_VEP_temp_Consequence_no_modifier, introgressed_variants_Ensembl_VEP_order)) %>% 
	dplyr::summarise_at(introgressed_variants_Ensembl_VEP_names, function(x) {paste(x, collapse=",")}) %>% 
	ungroup()
...
names_inter <- intersect(names(introgressed_variants_tb), names(introgressed_variants_Ensembl_VEP))
introgressed_variants_Ensembl_VEP <- introgressed_variants_tb %>% mutate_at(names_inter, as.character) %>% 
	left_join(introgressed_variants_Ensembl_VEP %>% mutate_at(names_inter, as.character))

# save all variants
write_tsv(introgressed_variants_Ensembl_VEP, gzfile("../../results/2a-annotate_variants_Ensembl_VEP/introgressed_variants_Ensembl_VEP.txt.gz"))

# save summ variants
introgressed_variants_Ensembl_VEP_summ <- introgressed_variants_Ensembl_VEP %>% 
	dplyr::select(seqnames, start, end, width, strand, starts_with("Variant"), contains("_summ_"))
write_tsv(introgressed_variants_Ensembl_VEP_summ, gzfile("../../results/2a-annotate_variants_Ensembl_VEP/introgressed_variants_Ensembl_VEP_summ.txt.gz"))
sort(table(introgressed_variants_Ensembl_VEP_summ$Ensembl_VEP_summ_Summary))

# visual checks
pdf("../../results/2a-annotate_variants_Ensembl_VEP/introgressed_variants_Ensembl_VEP_pie.pdf")
pie(rev(sort(table(introgressed_variants_Ensembl_VEP$Ensembl_VEP_summ_Category))))
dev.off()

pdf("../../results/2a-annotate_variants_Ensembl_VEP/introgressed_variants_Ensembl_VEP_summ_pie.pdf")
pie(rev(sort(table(introgressed_variants_Ensembl_VEP_summ$Ensembl_VEP_summ_Summary))))
dev.off()
