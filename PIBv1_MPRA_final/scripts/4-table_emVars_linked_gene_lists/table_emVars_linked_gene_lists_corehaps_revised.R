...
# load files
corehaps_variants <- as_tibble(fread("../../results/3-merge_all_variant_annotations/corehaps_variants_all_annotations_revised_summ.txt.gz"))

# Look up each variant and their associated and genes split by method
corehaps_variants_emVars <- corehaps_variants %>% 
	filter(!is.na(MPRA_K562_summ_Summary) | !is.na(MPRA_Jurkat_summ_Summary) | !is.na(MPRA_HepG2_summ_Summary))
...
corehaps_variants_emVars_linked_abbrev_variants <- corehaps_variants_emVars_linked %>% 
	...
	mutate(MPRA_HepG2_summ_Summary = gsub("_", " ", MPRA_HepG2_summ_Summary)) %>% 
	mutate(MPRA_HepG2_summ_Description_skew_log10FDR = ifelse(!is.na(MPRA_HepG2_summ_Summary), signif(MPRA_HepG2_summ_Description_skew_log10FDR, 3), NA)) %>% 
	mutate(MPRA_HepG2_summ_Description_skew_log2FC = ifelse(!is.na(MPRA_HepG2_summ_Summary), signif(MPRA_HepG2_summ_Description_skew_log2FC, 3), NA))

corehaps_variants_emVars_linked_abbrev_variants_named <- corehaps_variants_emVars_linked_abbrev_variants
names(corehaps_variants_emVars_linked_abbrev_variants_named) <- c("Variant ID", "MPRA K562", "MPRA K562 log10FDR", "MPRA K562 log2FC", "MPRA Jurkat", "MPRA Jurkat log10FDR", "MPRA Jurkat log2FC", "MPRA HepG2", "MPRA HepG2 log10FDR", "MPRA HepG2 log2FC", "ABC score list", "ABC gene symbol list", "ABC biosample list", "ABC category list", "EpiMap score list", "EpiMap gene symbol list", "EpiMap group list", "Roadmap Epigenomics score list", "Roadmap Epigenomics gene symbol list", "Roadmap Epigenomics biosample list", "Roadmap Epigenomics category list", "CADD score", "phyloP score")
write_tsv(corehaps_variants_emVars_linked_abbrev_variants_named, "../../results/4-table_emVars_linked_gene_lists/Supplementary_Table_SX-Corehaps_introgressed_MPRA_emVar_variants_with_gene_enhancer_links_in_final_dataset_revised.txt")
...
write_tsv(corehaps_variants_emVars_linked_variant_gene_pairs_named, "../../results/4-table_emVars_linked_gene_lists/Supplementary_Table_SX-Pairs_of_corehaps_introgressed_MPRA_emVar_variants_and_linked_genes_in_final_dataset_revised.txt")
...
write_tsv(corehaps_variants_emVars_linked_genes_named, "../../results/4-table_emVars_linked_gene_lists/Supplementary_Table_SX-Genes_with_corehaps_introgressed_MPRA_emVars_in_final_dataset_revised.txt")
...
write_tsv(corehaps_variants_emVars_linked_genes_collapse_named, "../../results/4-table_emVars_linked_gene_lists/Supplementary_Table_SX-Gene_lists_by_count_of_corehaps_introgressed_MPRA_emVars_in_final_dataset_revised.txt")
