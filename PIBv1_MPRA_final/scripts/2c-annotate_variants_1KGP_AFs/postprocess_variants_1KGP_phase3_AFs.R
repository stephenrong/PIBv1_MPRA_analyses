...
# load VCF
introgressed_variants_1KGP_phase3_AFs <- read.vcfR("../../results/2c-annotate_variants_1KGP_AFs/introgressed_variants-ALL.wgs.phase3_shapeit2_mvncall_integrated_v5c.20130502.sites.vcf.gz")

# split INFO
introgressed_variants_1KGP_phase3_AFs <- vcfR2tidy(introgressed_variants_1KGP_phase3_AFs, info_only=TRUE)$fix

# filter SNPs
introgressed_variants_1KGP_phase3_AFs <- introgressed_variants_1KGP_phase3_AFs %>% 
	filter(REF %in% c("A", "C", "G", "T"), ALT %in% c("A", "C", "G", "T"))

# add prefix and save names
names(introgressed_variants_1KGP_phase3_AFs)[c(3,6:length(introgressed_variants_1KGP_phase3_AFs))] <- paste("1KGP_phase3_orig", names(introgressed_variants_1KGP_phase3_AFs)[c(3,6:length(introgressed_variants_1KGP_phase3_AFs))], sep="_")
introgressed_variants_1KGP_phase3_AFs_names <- names(introgressed_variants_1KGP_phase3_AFs)[c(3,6:length(introgressed_variants_1KGP_phase3_AFs))]
...
introgressed_variants_1KGP_phase3_AFs <- introgressed_variants_1KGP_phase3_AFs %>% 
	...
	dplyr::select(seqnames, start, end, width, strand, VariantID, VariantCHROM, VariantPOS, VariantREF, VariantALT, all_of(introgressed_variants_1KGP_phase3_AFs_names))
...
names_temp <- intersect(names(introgressed_variants_tb), names(introgressed_variants_1KGP_phase3_AFs))
introgressed_variants_1KGP_phase3_AFs <- introgressed_variants_tb %>% mutate_at(names_temp, as.character)%>% 
	left_join(introgressed_variants_1KGP_phase3_AFs %>% mutate_at(names_temp, as.character))

# save all variants
write_tsv(introgressed_variants_1KGP_phase3_AFs, gzfile("../../results/2c-annotate_variants_1KGP_AFs/introgressed_variants_1KGP_phase3_AFs.txt.gz"))

# save summ variants
introgressed_variants_1KGP_phase3_AFs_summ <- introgressed_variants_1KGP_phase3_AFs %>% 
	dplyr::select(seqnames, start, end, width, strand, starts_with("Variant"), contains("_summ_"))
write_tsv(introgressed_variants_1KGP_phase3_AFs_summ, gzfile("../../results/2c-annotate_variants_1KGP_AFs/introgressed_variants_1KGP_phase3_AFs_summ.txt.gz"))
