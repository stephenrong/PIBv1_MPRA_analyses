...
# load gr
introgressed_variants_BBJ_finemap_ucsc_custom_track <- as_tibble(fread("../../results/2m-annotate_variants_UKBB_BBJ_finemap/introgressed_variants_BBJ_finemap_summ.txt.gz"))
introgressed_variants_BBJ_finemap_ucsc_custom_track <- introgressed_variants_BBJ_finemap_ucsc_custom_track %>% 
	filter(!is.na(BBJ_finemap_summ_Summary))

# create bed
custom_palette = as.vector(paletteer_d("RColorBrewer::Set1"))

introgressed_variants_BBJ_finemap_ucsc_custom_track_bed <- introgressed_variants_BBJ_finemap_ucsc_custom_track %>% 
	mutate(
		bed_chrom=seqnames,
...
create_ucsc_custom_bed_track(
	input_tb=introgressed_variants_BBJ_finemap_ucsc_custom_track_bed, 
	output_bed="../../results/2m-annotate_variants_UKBB_BBJ_finemap/introgressed_variants_BBJ_finemap_ucsc_custom_track.bed", 
