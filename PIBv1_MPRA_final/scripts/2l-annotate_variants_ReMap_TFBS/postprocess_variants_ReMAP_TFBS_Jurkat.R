...
# load VCF 
introgressed_variants_ReMap_TFBS_Jurkat <- as_tibble(fread("../../results/1a-preprocess_PIBv1_MPRA_final/introgressed_variants.txt.gz")) %>% 
...
# overlap
overlap_temp <- as_tibble(findOverlaps(GRanges(introgressed_variants_ReMap_TFBS_Jurkat), GRanges(ReMap_TFBS_Jurkat_tb)))
overlap_temp$VariantID <- introgressed_variants_ReMap_TFBS_Jurkat$VariantID[overlap_temp$queryHits]
...
introgressed_variants_ReMap_TFBS_Jurkat <- introgressed_variants_ReMap_TFBS_Jurkat %>% 
	left_join(overlap_temp)

# save names
introgressed_variants_ReMap_TFBS_Jurkat_names <- names(introgressed_variants_ReMap_TFBS_Jurkat)[11:length(introgressed_variants_ReMap_TFBS_Jurkat)]
...
introgressed_variants_ReMap_TFBS_Jurkat <- introgressed_variants_ReMap_TFBS_Jurkat %>% 
	...
	dplyr::summarise_at(introgressed_variants_ReMap_TFBS_Jurkat_names, function(x) {paste(x, collapse=",")}) %>% 
	mutate_at(introgressed_variants_ReMap_TFBS_Jurkat_names, function(x) {ifelse(x=="NA", NA, x)}) %>% 
	ungroup()
...
introgressed_variants_tb <- as_tibble(fread("../../results/1a-preprocess_PIBv1_MPRA_final/introgressed_variants.txt.gz")) %>% 
	dplyr::select(seqnames, start, end, width, strand, VariantID, VariantCHROM, VariantPOS, VariantREF, VariantALT)
names_inter <- intersect(names(introgressed_variants_tb), names(introgressed_variants_ReMap_TFBS_Jurkat))
introgressed_variants_ReMap_TFBS_Jurkat <- introgressed_variants_tb %>% mutate_at(names_inter, as.character) %>% 
	left_join(introgressed_variants_ReMap_TFBS_Jurkat %>% mutate_at(names_inter, as.character))

# save all variants
write_tsv(introgressed_variants_ReMap_TFBS_Jurkat, gzfile("../../results/2l-annotate_variants_ReMap_TFBS/introgressed_variants_ReMap_TFBS_Jurkat.txt.gz"))

# save summ variants
introgressed_variants_ReMap_TFBS_Jurkat_summ <- introgressed_variants_ReMap_TFBS_Jurkat %>% 
	dplyr::select(seqnames, start, end, width, strand, starts_with("Variant"), contains("_summ_"))
write_tsv(introgressed_variants_ReMap_TFBS_Jurkat_summ, gzfile("../../results/2l-annotate_variants_ReMap_TFBS/introgressed_variants_ReMap_TFBS_Jurkat_summ.txt.gz"))
sort(table(introgressed_variants_ReMap_TFBS_Jurkat_summ$ReMap_TFBS_Jurkat_summ_Summary))
