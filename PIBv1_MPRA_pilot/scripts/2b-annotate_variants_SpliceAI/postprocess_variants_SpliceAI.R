...
# load VCF
adaptive_variants_SpliceAI <- as_tibble(read.vcfR("../../results/2b-annotate_variants_SpliceAI/adaptive_variants-SpliceAI_raw_hg19.vcf.gz")@fix) 

# add GRange and Variant cols
adaptive_variants_SpliceAI <- adaptive_variants_SpliceAI %>% 
...
transId_map <- deframe(transId(sort(unique(adaptive_variants_SpliceAI$SpliceAI_raw_orig_SYMBOL)), transTo=c("symbol"), keepNA=TRUE, unique=TRUE, hgVersion="v38"))
adaptive_variants_SpliceAI <- adaptive_variants_SpliceAI %>% 
...
names_temp <- intersect(names(adaptive_variants_tb), names(adaptive_variants_SpliceAI))
adaptive_variants_SpliceAI <- adaptive_variants_tb %>% mutate_at(names_temp, as.character)%>% 
	left_join(adaptive_variants_SpliceAI %>% mutate_at(names_temp, as.character))

# save all variants
write_tsv(adaptive_variants_SpliceAI, gzfile("../../results/2b-annotate_variants_SpliceAI/adaptive_variants_SpliceAI.txt.gz"))

# save summ variants
adaptive_variants_SpliceAI_summ <- adaptive_variants_SpliceAI %>% 
	dplyr::select(seqnames, start, end, width, strand, starts_with("Variant"), contains("_summ_"))
write_tsv(adaptive_variants_SpliceAI_summ, gzfile("../../results/2b-annotate_variants_SpliceAI/adaptive_variants_SpliceAI_summ.txt.gz"))
sort(table(adaptive_variants_SpliceAI_summ$SpliceAI_raw_summ_Summary))

# visual check
pdf("../../results/2b-annotate_variants_SpliceAI/adaptive_variants_SpliceAI_pie.pdf")
plot(sort(adaptive_variants_SpliceAI$SpliceAI_raw_summ_Score), ylab="SpliceAI max score")
dev.off()

pdf("../../results/2b-annotate_variants_SpliceAI/adaptive_variants_SpliceAI_summ_pie.pdf")
pie(rev(sort(table(adaptive_variants_SpliceAI_summ$SpliceAI_raw_summ_Summary))))
dev.off()
