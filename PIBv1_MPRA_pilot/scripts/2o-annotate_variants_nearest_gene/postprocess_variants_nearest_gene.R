...
# get nearest
overlap_temp <- as_tibble(distanceToNearest(GRanges(adaptive_variants_nearest_gene), GRanges(ucsc_knownGene_abbrev), select="all"))
overlap_temp$VariantID <- adaptive_variants_nearest_gene$VariantID[overlap_temp$queryHits]
...
adaptive_variants_nearest_gene <- adaptive_variants_nearest_gene %>% 
	left_join(overlap_temp)

# save names
adaptive_variants_nearest_gene_names <- names(adaptive_variants_nearest_gene)[11:length(adaptive_variants_nearest_gene)]
...
adaptive_variants_nearest_gene <- adaptive_variants_nearest_gene %>% 
	...
	dplyr::summarise_at(adaptive_variants_nearest_gene_names, function(x) {paste(x, collapse=",")}) %>% 
	mutate_at(adaptive_variants_nearest_gene_names, function(x) {ifelse(x=="NA", NA, x)}) %>% 
	ungroup()
...
names_inter <- intersect(names(adaptive_variants_tb), names(adaptive_variants_nearest_gene))
adaptive_variants_nearest_gene <- adaptive_variants_tb %>% mutate_at(names_inter, as.character) %>% 
	left_join(adaptive_variants_nearest_gene %>% mutate_at(names_inter, as.character))

# save all variants
write_tsv(adaptive_variants_nearest_gene, gzfile("../../results/2o-annotate_variants_nearest_gene/adaptive_variants_nearest_gene.txt.gz"))

# save summ variants
adaptive_variants_nearest_gene_summ <- adaptive_variants_nearest_gene %>% 
	dplyr::select(seqnames, start, end, width, strand, starts_with("Variant"), contains("_summ_"))
write_tsv(adaptive_variants_nearest_gene_summ, gzfile("../../results/2o-annotate_variants_nearest_gene/adaptive_variants_nearest_gene_summ.txt.gz"))
sort(table(adaptive_variants_nearest_gene_summ$nearest_gene_summ_Symbol))
