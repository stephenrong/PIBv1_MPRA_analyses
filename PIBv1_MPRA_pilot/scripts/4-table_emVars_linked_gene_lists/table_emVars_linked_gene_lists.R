...
adaptive_variants_emVars <- adaptive_variants %>% 
	filter(!is.na(MPRA_K562_summ_Summary) | !is.na(MPRA_Jurkat_summ_Summary))

adaptive_variants_emVars_linked <- adaptive_variants_emVars %>% 
...
adaptive_variants_emVars_linked_abbrev_variants <- adaptive_variants_emVars_linked %>% 
...
adaptive_variants_emVars_linked_variant_gene_pairs <- adaptive_variants_emVars_linked %>% 
...
adaptive_variants_emVars_linked_variant_gene_pairs_named <- adaptive_variants_emVars_linked_variant_gene_pairs
names(adaptive_variants_emVars_linked_variant_gene_pairs_named) <- c("Variant ID", "Gene symbol", "Gene-enhancer methods count", "Gene-enhancer methods list", "CADD score", "phyloP score")
write_tsv(adaptive_variants_emVars_linked_variant_gene_pairs_named, "../../results/4-table_emVars_linked_gene_lists/Supplementary_Table_SX-Pairs_of_adaptive_introgressed_MPRA_emVar_variants_and_linked_genes_in_pilot_dataset.txt")

# Look up for a given gene which variants are linked
adaptive_variants_emVars_linked_genes <- adaptive_variants_emVars_linked_variant_gene_pairs %>% 
...
adaptive_variants_emVars_linked_genes_named <- adaptive_variants_emVars_linked_genes
names(adaptive_variants_emVars_linked_genes_named) <- c("Gene symbol", "emVar+linked variant count", "Variant ID list")
write_tsv(adaptive_variants_emVars_linked_genes_named, "../../results/4-table_emVars_linked_gene_lists/Supplementary_Table_SX-Genes_with_adaptive_introgressed_MPRA_emVars_in_pilot_dataset.txt")

# Look up which genes have the most links
adaptive_variants_emVars_linked_genes_collapse <- adaptive_variants_emVars_linked_genes %>% 
	group_by(`Linked variant count`) %>% summarise(`Count` = dplyr::n(), `Genes` = paste(`Gene`, collapse=",")) %>% ungroup() %>% 
	arrange(desc(`Linked variant count`))
adaptive_variants_emVars_linked_genes_collapse_named <- adaptive_variants_emVars_linked_genes_collapse
names(adaptive_variants_emVars_linked_genes_collapse_named) <- c("emVar+linked variant count", "Gene count", "Gene symbol list")
write_tsv(adaptive_variants_emVars_linked_genes_collapse_named, "../../results/4-table_emVars_linked_gene_lists/Supplementary_Table_SX-Gene_lists_by_count_of_adaptive_introgressed_MPRA_emVars_in_pilot_dataset.txt")
