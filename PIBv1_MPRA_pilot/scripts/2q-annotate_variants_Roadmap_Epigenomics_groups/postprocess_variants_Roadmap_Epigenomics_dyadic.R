...
adaptive_variants_Roadmap_Epigenomics_dyadic_names <- names(Roadmap_Epigenomics_dyadic_tb)[6:length(Roadmap_Epigenomics_dyadic_tb)]

# overlap variants and Roadmap_Epigenomics_dyadic
adaptive_variants_Roadmap_Epigenomics_dyadic <- as_tibble(find_overlaps(GRanges(adaptive_variants_tb), GRanges(Roadmap_Epigenomics_dyadic_tb)))

# Roadmap_Epigenomics_dyadic names
adaptive_variants_Roadmap_Epigenomics_dyadic_names <- names(adaptive_variants_Roadmap_Epigenomics_dyadic)[11:length(adaptive_variants_Roadmap_Epigenomics_dyadic)]

# collapse by group
adaptive_variants_Roadmap_Epigenomics_dyadic_collapse <- adaptive_variants_Roadmap_Epigenomics_dyadic %>% 
	group_by(seqnames, start, end, width, strand, VariantID, VariantCHROM, VariantPOS, VariantREF, VariantALT) %>% 
	dplyr::summarise_at(adaptive_variants_Roadmap_Epigenomics_dyadic_names, function(x) {paste(x, collapse=",")}) %>% 
	ungroup() %>% 
...
# pivot by group
adaptive_variants_Roadmap_Epigenomics_dyadic_pivot <- adaptive_variants_Roadmap_Epigenomics_dyadic %>% 
...
# join together
adaptive_variants_Roadmap_Epigenomics_dyadic <- full_join(adaptive_variants_Roadmap_Epigenomics_dyadic_collapse, adaptive_variants_Roadmap_Epigenomics_dyadic_pivot)
...
adaptive_variants_tb <- as_tibble(fread("../../results/1a-preprocess_PIBv1_MPRA_pilot/adaptive_variants.txt.gz")) %>% 
	dplyr::select(seqnames, start, end, width, strand, VariantID, VariantCHROM, VariantPOS, VariantREF, VariantALT)
names_temp <- intersect(names(adaptive_variants_tb), names(adaptive_variants_Roadmap_Epigenomics_dyadic))
adaptive_variants_Roadmap_Epigenomics_dyadic <- adaptive_variants_tb %>% mutate_at(names_temp, as.character)%>% 
	left_join(adaptive_variants_Roadmap_Epigenomics_dyadic %>% mutate_at(names_temp, as.character))

# save all variants
write_tsv(adaptive_variants_Roadmap_Epigenomics_dyadic, gzfile("../../results/2q-annotate_variants_Roadmap_Epigenomics_groups/adaptive_variants_Roadmap_Epigenomics_dyadic.txt.gz"))

# save summ variants
adaptive_variants_Roadmap_Epigenomics_dyadic_summ <- adaptive_variants_Roadmap_Epigenomics_dyadic %>% 
	dplyr::select(seqnames, start, end, width, strand, starts_with("Variant"), contains("_summ_"))
write_tsv(adaptive_variants_Roadmap_Epigenomics_dyadic_summ, gzfile("../../results/2q-annotate_variants_Roadmap_Epigenomics_groups/adaptive_variants_Roadmap_Epigenomics_dyadic_summ.txt.gz"))
sort(table(adaptive_variants_Roadmap_Epigenomics_dyadic_summ$Roadmap_Epigenomics_dyadic_summ_Summary))
