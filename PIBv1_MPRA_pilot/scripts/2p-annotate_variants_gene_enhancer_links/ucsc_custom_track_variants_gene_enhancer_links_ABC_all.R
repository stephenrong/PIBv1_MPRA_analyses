...
# load ABC_all
ABC_all_gene_enhancer_links <- as_tibble(fread("../../results/2p-annotate_variants_gene_enhancer_links/adaptive_variants_ABC_interact_all.txt.gz"))
ABC_all_gene_enhancer_links_abbrev <- ABC_all_gene_enhancer_links %>% 
	GRanges() %>% seqinfo_fix("UCSC", "hg19") %>% as_tibble()

ABC_all_gene_enhancer_links_abbrev <- ABC_all_gene_enhancer_links_abbrev %>% 
...
ABC_all_gene_enhancer_links_emVar <- ABC_all_gene_enhancer_links_abbrev %>% 
...
ABC_all_gene_enhancer_links_emVar <- ABC_all_gene_enhancer_links_emVar %>% 
	rowwise() %>% 
	mutate(
...
create_ucsc_custom_interact_track(
	input_tb=ABC_all_gene_enhancer_links_emVar,
	output_interact="../../results/2p-annotate_variants_gene_enhancer_links/adaptive_variants_ABC_all_emVar_ucsc_custom_track.interact", 
	name="ABC-All+emVar gene enhancer links", description="ABC-All+emVar gene enhancer links", interactDirectional="true", maxHeightPixels="200:100:50", visibility="full")
