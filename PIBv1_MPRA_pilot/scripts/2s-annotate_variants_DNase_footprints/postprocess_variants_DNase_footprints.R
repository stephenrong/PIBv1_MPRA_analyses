...
# load VCF 
adaptive_variants_DNase_footprints <- as_tibble(fread("../../results/1a-preprocess_PIBv1_MPRA_pilot/adaptive_variants.txt.gz")) %>% 
...
# overlap
overlap_temp <- as_tibble(findOverlaps(GRanges(adaptive_variants_DNase_footprints), DNase_footprints_gr))
overlap_temp$VariantID <- adaptive_variants_DNase_footprints$VariantID[overlap_temp$queryHits]
...
adaptive_variants_DNase_footprints <- adaptive_variants_DNase_footprints %>% 
	left_join(overlap_temp)

# save names
adaptive_variants_DNase_footprints_names <- names(adaptive_variants_DNase_footprints)[11:length(adaptive_variants_DNase_footprints)]
...
adaptive_variants_DNase_footprints <- adaptive_variants_DNase_footprints %>% 
	...
	dplyr::summarise_at(adaptive_variants_DNase_footprints_names, function(x) {paste(x, collapse=",")}) %>% 
	mutate_at(adaptive_variants_DNase_footprints_names, function(x) {ifelse(x=="NA", NA, x)}) %>% 
	ungroup()
...
adaptive_variants_tb <- as_tibble(fread("../../results/1a-preprocess_PIBv1_MPRA_pilot/adaptive_variants.txt.gz")) %>% 
	dplyr::select(seqnames, start, end, width, strand, VariantID, VariantCHROM, VariantPOS, VariantREF, VariantALT)
names_inter <- intersect(names(adaptive_variants_tb), names(adaptive_variants_DNase_footprints))
adaptive_variants_DNase_footprints <- adaptive_variants_tb %>% mutate_at(names_inter, as.character) %>% 
	left_join(adaptive_variants_DNase_footprints %>% mutate_at(names_inter, as.character))

# save all variants
write_tsv(adaptive_variants_DNase_footprints, gzfile("../../results/2s-annotate_variants_DNase_footprints/adaptive_variants_DNase_footprints.txt.gz"))

# save summ variants
adaptive_variants_DNase_footprints_summ <- adaptive_variants_DNase_footprints %>% 
	dplyr::select(seqnames, start, end, width, strand, starts_with("Variant"), contains("_summ_"))
write_tsv(adaptive_variants_DNase_footprints_summ, gzfile("../../results/2s-annotate_variants_DNase_footprints/adaptive_variants_DNase_footprints_summ.txt.gz"))
sort(table(adaptive_variants_DNase_footprints_summ$DNase_footprints_summ_Summary))
